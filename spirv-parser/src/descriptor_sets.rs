    ...
    for instruction in doc.instructions.iter() {
        ...
        let (desc_ty, readonly, array_count) =
            descriptor_infos(doc, pointed_ty, false).expect(&format!(
                "Couldn't find relevant type for uniform `{}` (type {}, maybe \
                 unimplemented)",
                name, pointed_ty
            ));
            ...
            match i {
                &parse::Instruction::TypeStruct { result_id, .. } if result_id == pointed_ty => {
                    ...
                    let is_ssbo = doc.instructions
                        ...
                        .filter_map(|i| match i {
                            &parse::Instruction::Decorate {
                                target_id,
                                decoration: enums::Decoration::DecorationBufferBlock,
                                ..
                            ...
                            &parse::Instruction::Decorate {
                                target_id,
                                decoration: enums::Decoration::DecorationBlock,
                                ..
                            } if target_id == pointed_ty =>
                            {
                                Some(false)
                        ...
                        .next()
                        .expect(
                            "Found a buffer uniform with neither the Block nor BufferBlock \
                             decorations",
                        );
                ...
                } if result_id == pointed_ty =>
                {
                    ...
                    if let &enums::Dim::DimSubpassData = dim {
                        ...
                        assert!(
                            if let &enums::ImageFormat::ImageFormatUnknown = format {
                                true
                            } else {
                                false
                            },
                        ...
                        let desc = format!(
                            "DescriptorDescTy::{}(DescriptorImageDesc {{
                        sampled: {},
                        dimensions: {},
                        format: None,       // TODO: specify format if known
                        multisampled: {},
                ...
                &parse::Instruction::TypeSampledImage {
                    result_id,
                    image_type_id,
                } if result_id == pointed_ty =>
                {
